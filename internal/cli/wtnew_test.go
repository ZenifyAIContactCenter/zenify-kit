...
func TestWtNew_Integration_CreatesWorktree(t *testing.T) {
	...
	stateRaw, err := os.ReadFile(filepath.Join(root, ".wt", "state.json")) //nolint:gosec // G304 -- path is computed internally by this tool from its own config/workspace state, not externally-tainted input
...
func TestWtNew_SeedsIdentityEnv(t *testing.T) {
	...
	b, err := os.ReadFile(filepath.Join(root, ".worktrees", "id-task", ".env")) //nolint:gosec // G304 -- path is computed internally by this tool from its own config/workspace state, not externally-tainted input
	if err != nil {
		t.Fatal(err)
	}
	got := string(b)
...
func initGitRepoWithPortCount(t *testing.T, portCount int) string {
	...
	_ = os.WriteFile(filepath.Join(root, ".claude", "worktree.json"), []byte(cfg), 0o600)
	_ = os.WriteFile(filepath.Join(root, "seed.txt"), []byte("hi"), 0o600)
	run("add", "-A")
	run("commit", "-q", "-m", "init")
	return root
...
func TestWtNew_PortCountGreaterThanOne_SeedsBlockAndBase(t *testing.T) {
	...
	env, err := os.ReadFile(filepath.Join(root, ".worktrees", "hub-task", ".env")) //nolint:gosec // G304 -- path is computed internally by this tool from its own config/workspace state, not externally-tainted input
	if err != nil {
		t.Fatal(err)
	}
	got := string(env)
...
func TestWtNew_PortCountAbsent_NoRegression(t *testing.T) {
	...
	env, err := os.ReadFile(filepath.Join(root, ".worktrees", "plain-task", ".env")) //nolint:gosec // G304 -- path is computed internally by this tool from its own config/workspace state, not externally-tainted input
	if err != nil {
		t.Fatal(err)
	}
	got := string(env)
