...
func newDownCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "down",
		Short: "Offboard: gỡ znf global hooks, .worktrees/ excludes, và owned settings skeletons (preview mặc định; --apply để thực thi)",
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			mode := "APPLY"
			if dryRun {
				mode = "PREVIEW (dry-run — dùng --apply để thực thi)"
			}
			...
			if home, herr := os.UserHomeDir(); herr == nil && home != "" {
				...
				default:
					_, _ = fmt.Fprintln(w, "  hooks: không có znf hook nào để gỡ")
				}
			}

			// 2 + 3. Mỗi repo: exclude line + owned settings skeleton.
			...
			_, _ = fmt.Fprintln(w, "(down KHÔNG đụng cloned repo, .zenify/, hay docs store)")
			return nil
		},
	}
	cmd.Flags().BoolVar(&applyFlag, "apply", false, "thực thi gỡ (mặc định chỉ preview)")
