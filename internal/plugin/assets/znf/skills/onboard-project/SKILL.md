...
at minimum confirm it exists in `package.json` scripts / `Makefile` / the equivalent. Do not copy
a command out of a README, a sibling project, or an `understand-codebase` summary and record it
as fact. If you genuinely cannot verify one, write it with an explicit `(unverified)` marker
rather than stating it plainly. Same rule for paths, aliases, and structural claims — mark
anything inferred rather than observed. A wrong launch command silently disables `/run`'s
behavioural verification; a wrong structural claim is read and believed every session.

**Date-stamp the map.** End `CLAUDE.md` with a line like
`Map verified 2026-07-27 against commit <short-sha>` (`git rev-parse --short HEAD`) — the
cheapest defence against a map that was right when written and silently drifted. See
`references/onboarding-rationale.md` for the real incident this defends against.
...
- **`znf:code-reviewer`, `znf:scout`, `znf:ui-verifier`** agents (shipped by the plugin, model pinned per agent — `code-reviewer` at `claude-opus-4-8` full ID, `/ship` scales it down for a small diff; the other two sonnet). Three, not more — `db-schema-fetcher` and `project-manager` were retired for zero use.
- **Model routing rule** — a skill always runs on the session model; only a dispatched subagent can pin its own model (frontmatter or the dispatch call's `model` param), defaulting to `CLAUDE_CODE_SUBAGENT_MODEL` (`sonnet`, set by `zenify up`). A skill can never "escalate to Opus" on its own — route that step through a subagent dispatched with `model: 'opus'` instead. Full mechanics: `references/onboarding-rationale.md`.
- **`/cook`, `/fix`** (1-verb pipeline), **`/ground`** (anti-fabrication)
...
7. **DB read access config**: if this project has a DB, give it a documented read-only accessor and record the command in CLAUDE.md — a wrapper taking credentials from a designated store, not a raw connection string, not `grep <VAR> .env`. `/ground` reads this recipe. Never record the credential itself.

There is no Step 8 — status lives in spec/plan files under `docs/superpowers/` and the SDD
ledger at `.znf/sdd/<plan>/progress.md`, both a side effect of doing the work. See
`references/onboarding-rationale.md` for why.

## References

- `references/onboarding-rationale.md` — the date-stamp incident, full model-routing mechanics, and why there is no Step 8.
