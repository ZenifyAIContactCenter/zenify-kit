...
Every `async` handler is a `try { … } catch (err) { return next(err); }` — an unhandled rejection
never reaches the error middleware. Pass the caught error to `next` unchanged; re-wrapping as
`new Error(msg)` drops `statusCode` and the stable `errorCode`. `AppError` shape and the full
handler: `references/async-errors.md`.

## Cross-refs

- The Mongo/Mongoose model behind `db.mongo.*` above — schema and index
  design, safe field rollout — → `znf:mongoose-modeling`.
...
- `references/async-errors.md` — the try/catch handler and `AppError`, in full.
